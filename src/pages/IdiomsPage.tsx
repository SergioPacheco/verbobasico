...
export function IdiomsPage({ onBack, onNavigate }: IdiomsPageProps) {
  ...
  if (mode === 'quiz' && quizDone) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-50 via-white to-red-50">
        <Header title="🗣️ Quiz de Expressões" onNavigate={onNavigate} showBack onBack={() => setMode('browse')} />
        <div className="flex items-center justify-center px-4 py-10">
          <div className="card max-w-sm w-full text-center animate-slide-up">
            ...
            <button onClick={() => setMode('browse')} className="btn-secondary w-full">📚 Ver lista</button>
          </div>
        </div>
      </div>
    );
  ...
  if (mode === 'quiz' && quizQueue.length > 0) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-50 via-white to-red-50">
        <Header
          title="🗣️ Quiz de Expressões"
          subtitle={`${quizIdx + 1}/${quizQueue.length}`}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-white to-red-50">
      <Header
        title="🗣️ Expressões"
        subtitle={`${idioms.length} expressões`}
        onNavigate={onNavigate}
        showBack
        onBack={onBack}
      />
            ...
            {(Object.keys(CATEGORY_LABELS) as Category[]).map((cat) => (
              <button
                ...
                className={`flex-shrink-0 text-xs px-3 py-1.5 rounded-full font-medium transition-colors ${
                  category === cat ? 'bg-spain-red text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                }`}
              >
                {CATEGORY_LABELS[cat]}
              </button>
